...
const { appLocales, DEFAULT_LOCALE } = require('../src/i18n/locale');
...
for (const locale of appLocales) {
  ...
  const translationFileName = `src/i18n/translation/${locale}.json`;
  try {
...
memoryTask.then((files) => {
  ...
  extractTask.then(() => {
    ...
    for (const locale of appLocales) {
      translationFileName = `src/i18n/translation/${locale}.json`;
      localeTaskDone = task(
